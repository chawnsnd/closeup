...
export default {
  ...
  methods: {
    ...
    clickPOI(place) {
      ...
      var lonlat = new Tmap.LonLat(place.frontLon, place.frontLat).transform(
        "EPSG:3857",
        "EPSG:3857"
      );
    ...
    recommand(places) {
      ...
      var a_lonLat = new Tmap.LonLat(
        Number(placeA.frontLon),
        Number(placeA.frontLat)
      ); //시작 좌표입니다.
      var b_lonLat = new Tmap.LonLat(
        Number(placeB.frontLon),
        Number(placeB.frontLat)
      ); //도착 좌표입니다.
      var optionObj = {
      ...
      exampleSocket.onopen = function(event) {
        ...
        exampleSocket.send(JSON.stringify({'a_lonlat' : a_lonLat,'b_lonlat' : b_lonLat}));
      };
    },
    ...
    setBound(places) {
      for (var idx in places) {
        this.bounds.extend(
          new Tmap.LonLat(places[idx].frontLon, places[idx].frontLat)
        );
      }
    ...
    ajaxPOISearch(lon, lat, categories) {
      ...
      $.ajax({
        method: "GET",
        url:
          "https://api2.sktelecom.com/tmap/pois/search/around?version=1&format=xml&callback=result", // 주변 POI 검색 api 요청 url입니다.
        async: false,
        data: {
